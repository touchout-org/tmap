...
* The four corners belong exclusively to the left/right zones, not the top/bottom ones: left/right zones render across the display's full height (see the zone-sizing math below), while top/bottom zones are horizontally inset to fit only the space between them, never reaching a corner. A left/right label's centered position can legitimately extend past the map's own top/bottom edge into that corner space; a top/bottom label's cannot extend past the map's own left/right edge, since there's no corresponding room there for it to use.
